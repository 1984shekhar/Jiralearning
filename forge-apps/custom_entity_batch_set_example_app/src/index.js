...
const useBulkSet = async () => {
    ...
    try {
        console.log('Trying to batch set save import logs partition: ', JSON.stringify(objectsToSet));
        await kvs.batchSet(objectsToSet);
    } catch (e) {
        console.error(`Could not batch set save import logs partition=${JSON.stringify(objectsToSet)} due to error: `, e);
    }
};
