...
jira = JIRA(server="http://localhost:8080",basic_auth=('cpandey', 'administrator'))
# Get all projects viewable by anonymous users.
projects = jira.projects()
for project in projects:
    for x in range(3):
        jira.create_issue(project=project.key, summary='New issue from jira-python',
                              description='Look into this one', issuetype={'name': 'Bug'})
# Sort available project keys, then return the second, third, and fourth keys.
keys = sorted(project.key for project in projects)[0:1]

# Get an issue.
issue = jira.issue("KP1-3")
